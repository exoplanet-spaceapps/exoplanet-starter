#!/usr/bin/env python3
"""
Add comprehensive test cell to 02_bls_baseline.ipynb
"""
import nbformat
import sys

def create_test_cell():
    """Create the comprehensive test cell content"""
    test_code = '''# 🧪 === COMPREHENSIVE TESTING SUITE === 🧪
...
def add_test_cell_to_notebook(notebook_path):
    """Add test cell to notebook at appropriate position"""
    ...
    # Find the best position for test cell (after imports, before main execution)
    # Look for cell with "TEST MODE" or similar marker
    test_cell_index = None
    for i, cell in enumerate(nb.cells):
        if cell.cell_type == 'code':
            source = cell.source.lower()
            if 'test' in source and 'mode' in source:
                test_cell_index = i
                print(f"Found existing test cell at index {i}")
                break

    # If no test cell found, look for execution section
    if test_cell_index is None:
        for i, cell in enumerate(nb.cells):
            if cell.cell_type == 'markdown':
                source = cell.source.lower()
                if '執行' in source or 'execution' in source or 'run' in source:
                    test_cell_index = i + 1
                    print(f"Inserting test cell after execution section at index {i+1}")
                    break

    # Default to inserting after cell 7 if no better position found
    if test_cell_index is None:
        test_cell_index = 8
        print(f"Using default position: index {test_cell_index}")

    # Create test cell
    test_code = create_test_cell()
    test_cell = nbformat.v4.new_code_cell(source=test_code)

    # Check if cell already exists and needs replacement
    if test_cell_index < len(nb.cells):
        existing_cell = nb.cells[test_cell_index]
        if existing_cell.cell_type == 'code' and '🧪' in existing_cell.source:
            print(f"Replacing existing test cell at index {test_cell_index}")
            nb.cells[test_cell_index] = test_cell
        else:
            print(f"Inserting new test cell at index {test_cell_index}")
            nb.cells.insert(test_cell_index, test_cell)
    else:
        print(f"Appending test cell at end")
    ...
    print("Test cell added successfully!")
    print(f"   Total cells now: {len(nb.cells)}")
    return True

if __name__ == '__main__':
    notebook_path = 'C:/Users/thc1006/Desktop/dev/exoplanet-starter/notebooks/02_bls_baseline.ipynb'

    try:
        success = add_test_cell_to_notebook(notebook_path)
